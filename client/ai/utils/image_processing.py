...
def fast_read_image_roi(filename,
                        orig_rect,
                        ImreadModes=cv2.IMREAD_COLOR,
                        normalization=False,
                        colorSpace='RGB'):
    '''
    快速读取图片的方法
...
def resize_image(image, resize_height, resize_width):
    ...
    width = image_shape[1]
    if (resize_height is
            None) and (resize_width is
                       None):  #错误写法：resize_height and resize_width is None
        return image
...
def get_bboxes_image(image, bboxes_list, resize_height=None,
                     resize_width=None):
    rects_list = bboxes2rects(bboxes_list)
    rect_images = get_rects_image(image, rects_list, resize_height,
                                  resize_width)
    return rect_images
...
def combime_save_image(orig_image, dest_image, out_dir, name, prefix):
    ...
    save_image(
        os.path.join(out_dir, "{}_src_{}.jpg".format(name, prefix)),
        dest_image)
